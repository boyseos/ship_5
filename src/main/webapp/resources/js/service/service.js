...
service = (() =>{
	...
	, navi_vue_js, router_js, admin_js, page_vue_js, compo_vue_js
	let init = ()=>{
		_ = $.ctx()
		img = $.img()
		css = $.css()
	...
	let recent_update = (x) => {
		...
		$.getJSON(_+'/articles/list/'+x.pxy.pageNum+'__'+x.pxy.pageSize,d =>{
			...
			$.each([
				{tag : '<li id="prev" class="page-item"><a class="page-link" href="#">Prev</a></li>', pageNum : -5, pageSize : pxy.pageSize, predicate : pxy.existPrev},
				{tag : '<li class="page-item"><a class="page-link" href="#">Next</a></li>', pageNum : +5, pageSize : pxy.pageSize, predicate : pxy.existNext}
				...
				,(i,j)=>{
					//if(j.predicate){
						...
						.appendTo('#pagenation')
						.click(()=>{
							recent_update({pxy : {pageNum : Math.floor(pxy.pageNum)+j.pageNum, pageSize : j.pageSize}})		
						})
					//}
			})
			let arr = new Array();
			for(let i=2 ; i >= -2; i--){
				if (pxy.pageNum+i >= 1 && pxy.pageNum+i <= pxy.endPage) arr.push(pxy.pageNum+i)
			}
			$.each(/*pxy.pageArr*/arr,(i,j)=>{//페이지네이션 1,2,3,4,5 추가
				$('<li class="page-item"><a class="page-link" href="#">'+j+'</a></li>')
				.insertAfter('#prev')				
				.click(()=>{
					alert(j)
					recent_update({pxy : {pageNum : j, pageSize : pxy.pageSize}})
				})
			})
			...
			$('#listSizeSelectDiv select option[value="'+pxy.pageSize+'"]').attr('selected', true)
			//$('#listSizeSelectDiv select').val(pxy.pageSize)
			$('#listSizeSelectDiv select').change(()=>{
				recent_update({pxy : {pageNum : Math.floor(pxy.pageNum), pageSize : $('#listSizeSelectDiv select').val()}})
			})
		})
	}
